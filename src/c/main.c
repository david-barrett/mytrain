...
static TextLayer *s_current_clock_layer; // New Clock Layer
...
static void main_window_load(Window *window) {
  ...
  // 1. TOP BAND: Route Label (White background, Black text)
  s_label_layer = text_layer_create(GRect(0, 0, bounds.size.w, 28));
  ...
  // 2. MIDDLE: Train Time (Transparent background, White text)
  s_time_layer = text_layer_create(GRect(0, 42, bounds.size.w, 50));
  ...
  // 3. MIDDLE: Status (Transparent background, White text)
  s_status_layer = text_layer_create(GRect(0, 92, bounds.size.w, 32));
  ...
  // 4. BOTTOM BAND: Current Clock (White background, Black text)
  s_current_clock_layer = text_layer_create(GRect(0, bounds.size.h - 34, bounds.size.w, 34));
  ...
  text_layer_set_font(s_current_clock_layer, fonts_get_system_font(FONT_KEY_GOTHIC_28_BOLD));
